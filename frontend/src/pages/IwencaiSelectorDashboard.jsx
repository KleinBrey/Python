...
import { ChevronLeft, ChevronRight, FileJson2, ListFilter, Loader2, Play, Search, Sparkles } from 'lucide-react';
...
const preferredColumns = ['股票代码', '股票简称', '最新价', '最新涨跌幅', '上市板块', '交易所'];
          ...
          <TableRow>
            {keys.map((key) => (
              <TableHead
                ...
                key={key}
                style={{ width: key.includes('名称') || key.includes('简称') ? 140 : 150 }}
              >
                {key}
              </TableHead>
            ))}
          ...
          {visibleRows.length ? visibleRows.map((row, rowIndex) => (
            <TableRow key={`${row.股票代码 || 'stock'}-${(currentPage - 1) * pageSize + rowIndex}`}>
              {keys.map((key) => {
                const value = row[key];
                return (
                  <TableCell className={key === '股票代码' ? 'sticky-code-column' : ''} key={key}>
                    <span title={value === null || value === undefined ? '' : String(value)}>
                      {formatValue(value)}
                    </span>
                  </TableCell>
                );
            ...
            <TableRow>
              <TableCell className="table-empty-state" colSpan={Math.max(1, keys.length)}>
                没有查询到股票，请尝试简化或放宽条件
              </TableCell>
            </TableRow>
          )}
          ...
          {rows.map((row, index) => {
            ...
            return (
              <TableRow
                ...
                tabIndex={0}
              >
                <TableCell title={String(row.股票代码 || '')}>{formatValue(row.股票代码)}</TableCell>
                <TableCell title={String(row.股票简称 || '')}>{formatValue(row.股票简称)}</TableCell>
...
export default function IwencaiSelectorDashboard() {
  ...
  useEffect(() => {
    ...
    if (!selectedStock || !rows.some((row) => row.股票代码 === selectedStock.股票代码)) {
        ...
        <div className="iwencai-query-body">
          ...
          <div className="iwencai-examples">
            ...
            {exampleQueries.map((example, index) => (
              <Button key={example} type="button" onClick={() => setQuery(example)} size="sm" variant="outline">
                示例 {index + 1}
              </Button>
            ))}
          </div>
